...
const validateBlock = (coinbase, txids) => {
  if (txids.length === 1) {
    throw new Error(
  ...
  if (
    coinbaseTx.ins[0].script.length < 2 ||
    coinbaseTx.ins[0].script.length > 100
  ) {
  ...
  if (!foundWitnessCommitment) {
    throw new Error(
      "Coinbase transaction does not contain a witness commitment or contains invalid witness commitment"
    );
  }
