...
const difficultyTarget = Buffer.from('0000ffff00000000000000000000000000000000000000000000000000000000', 'hex');
...
const mineBlock = (header) => {
  ...
  while (true) {
    ...
    hash = createHash('sha256').update(hashBuffer).digest().reverse().toString('hex');
...
try {
  ...
  const validTransactions = transactions.filter(transaction => /* Add validation logic here */);

  // Extract coinbase transaction and block header from data
  const coinbase = validTransactions.find(transaction => transaction.is_coinbase).vin[0];
  const header = validTransactions.find(transaction => !transaction.is_coinbase).vin[0].txid; // Assuming the first non-coinbase transaction's vin is the header
