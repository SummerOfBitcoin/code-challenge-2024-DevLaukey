...
function mine(block_header) {
  ...
  while (true) {
    // Concatenate the block header and nonce in the correct format
    const headerHash = Buffer.from(
      doubleHash(
        Buffer.concat([
          Buffer.from(block_header, "hex"),
          littleEndian(paddedNonce),
        ])
      ),
      "hex"
...
// Call the mine function with the block header
